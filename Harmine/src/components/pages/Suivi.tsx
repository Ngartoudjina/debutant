          ...
          >
            ...
            <div className="flex-1">
              <div className="flex items-center">
                ...
                <ChevronDown className={`w-4 h-4 ml-2 transition-transform ${expandedStep === index ? 'rotate-180' : ''}`} />
              </div>
              ...
              >
                ...
                {index === 2 && routeHistory.length > 0 && (
                  <div className="mt-3 space-y-2">
                    <p className="text-xs font-medium text-gray-700 dark:text-gray-300">Historique du trajet:</p>
...
const PackageDetails = ({ packageDetails }: { packageDetails: DeliveryStatus['packageDetails'] }) => {
        ...
        {isExpanded && (
          ...
          >
            <div className="flex justify-between items-center">
              <span className="text-xs text-gray-500 dark:text-gray-400">Poids:</span>
              <span className="text-sm font-medium text-gray-800 dark:text-gray-200">{packageDetails.weight}</span>
            </div>
            
            <div className="flex justify-between items-center">
              <span className="text-xs text-gray-500 dark:text-gray-400">Type:</span>
              <span className="text-sm font-medium text-gray-800 dark:text-gray-200">{packageDetails.type}</span>
            </div>
            
            <div className="flex justify-between items-center">
              <span className="text-xs text-gray-500 dark:text-gray-400">Fragile:</span>
            ...
            {packageDetails.specialInstructions && (
              <div className="pt-2 border-t border-gray-200 dark:border-gray-700">
                <span className="text-xs text-gray-500 dark:text-gray-400 block mb-1">Instructions spéciales:</span>
                <p className="text-sm text-gray-800 dark:text-gray-200">{packageDetails.specialInstructions}</p>
              </div>
            )}
          </motion.div>
        )}
...
const WeatherImpact = ({ weather }: { weather: WeatherData }) => {
  const getWeatherIcon = () => {
    switch (weather.condition) {
      ...
      case "stormy": return <CloudLightning className="w-6 h-6 text-purple-400" />;
    }
  };
...
const DeliveryOptionBadge = ({ option }: { option: DeliveryStatus['deliveryOption'] }) => {
  const optionConfig = {
    standard: {
      icon: Truck,
      text: "Standard",
  ...
  return (
    <div className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium text-white bg-gradient-to-r ${optionConfig[option].gradient}`}>
...
// Cloud component
const Cloud = ({ children, className, ...props }: React.ComponentProps<typeof motion.div>) => {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 0.8 }}
          ...
          >
            ...
            <div className="space-y-6">
              ...
              >
                ...
                <span className="text-gray-800 dark:text-gray-200">Activer les notifications</span>
              </button>
              <button
              ...
              >
                ...
                <span className="text-gray-800 dark:text-gray-200">Partager le suivi</span>
              </button>
              <button
              ...
              >
                ...
                <span className="text-gray-800 dark:text-gray-200">Contacter le support</span>
              </button>
...
const DeliveryTips = () => {
  ...
  return (
    <div className="bg-white/20 dark:bg-gray-800/30 backdrop-blur-sm rounded-xl p-5 border border-white/20 dark:border-gray-700/50">
      <h3 className="text-base font-medium text-gray-800 dark:text-white mb-4">Conseils et astuces</h3>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {tips.map((tip, index) => (
          ...
          >
            ...
            <h4 className="text-sm font-medium text-gray-800 dark:text-white mb-1">{tip.title}</h4>
...
const DeliveryAnalytics = () => {
  ...
  return (
    <div className="bg-white/20 dark:bg-gray-800/30 backdrop-blur-sm rounded-xl p-5 border border-white/20 dark:border-gray-700/50">
      <h3 className="text-base font-medium text-gray-800 dark:text-white mb-4">Évolution du temps estimé</h3>
      <div className="h-52">
...
const Suivi = () => {
    ...
    const fetchOrderData = async () => {
      try {
        ...
        if (orderId) {
          ...
          if (!response.ok) {
            throw new Error(result.error || 'Erreur lors de la récupération de la commande');
          }
          order = result.data;
        } else {
          const response = await fetch('http://localhost:5000/api/commandes/user', {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
              Authorization: `Bearer ${token}`,
        ...
        const courierResponse = await fetch(`http://localhost:5000/api/truecoursiers/${order.courierId}`, {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
          },
        ...
        if (!courierResponse.ok) {
          throw new Error(courierResult.error || 'Erreur lors de la récupération du coursier');
        }
        ...
        setDeliveryStatus({
          status: order.status,
          estimatedTime: new Date(order.scheduledDate).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }),
          ...
          courier: {
            ...
            profilePicture: courierResult.data.profilePicture?.secure_url || null,
        ...
        toast.error(error.message || "Impossible de charger les données de la commande");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const newLat = deliveryStatus.currentLocation.lat + (deliveryStatus.destination.lat - deliveryStatus.currentLocation.lat) * 0.05;
      const newLng = deliveryStatus.currentLocation.lng + (deliveryStatus.destination.lng - deliveryStatus.currentLocation.lng) * 0.05;
  ...
  const handleShareTracking = () => {
    const trackingUrl = `${window.location.origin}/track/${deliveryStatus?.trackingNumber || ''}`;
  ...
  const handleFeedbackSubmit = async (feedback: Feedback) => {
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok) {
        throw new Error(result.error || 'Erreur lors de la soumission de l\'avis');
      }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:via-slate-900 dark:to-indigo-950">
        <div className="text-center text-gray-700 dark:text-gray-300">
          ...
          <p>Aucune commande trouvée. Veuillez vérifier votre numéro de suivi.</p>
          <Link
  to="/reserv"
  className="
    bg-blue-600 
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:via-slate-900 dark:to-indigo-950 transition-colors duration-300 relative overflow-hidden">
      ...
      >
        ...
        <div className="text-center relative">
          ...
          >
            ...
            <p className="text-gray-700 dark:text-gray-300 text-sm">
              N° de suivi: <span className="font-semibold">{deliveryStatus.trackingNumber}</span>
            </p>
          </motion.div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <div className="hidden md:flex items-center gap-3">
              ...
              >
                <Bell
                  className={`w-5 h-5 ${
                    isNotificationsEnabled ? "text-green-400" : "text-gray-600 dark:text-gray-300"
                  }`}
                />
              </motion.button>
          ...
          {viewMode === 'map' ? (
            ...
            >
              ...
              <div className="bg-white/10 dark:bg-gray-900/50 rounded-xl overflow-hidden shadow-inner">
                <MapContainer
                  center={[deliveryStatus.currentLocation.lat, deliveryStatus.currentLocation.lng]}
                  zoom={mapZoom}
                ...
                >
                  ...
                  />
                  <Marker 
                    position={[deliveryStatus.currentLocation.lat, deliveryStatus.currentLocation.lng]}
                    icon={deliveryIcon}
                  >
                    <Popup>
                      <div className="p-2">
                        <p className="font-semibold">Position du livreur</p>
                        <p className="text-xs text-gray-600">Arrivée prévue dans {deliveryStatus.eta} min</p>
                      </div>
                    </Popup>
                  </Marker>
                  <Marker 
                    position={[deliveryStatus.destination.lat, deliveryStatus.destination.lng]}
                    icon={destinationIcon}
                  >
                    <Popup>
                      <div className="p-2">
                        <p className="font-semibold">Votre adresse</p>
                        <p className="text-xs text-gray-600">{deliveryStatus.deliveryAddress}</p>
                      </div>
                    </Popup>
                  </Marker>
                  <Polyline 
        ...
        >
          ...
          <DeliveryTimeline status={deliveryStatus.status} routeHistory={deliveryStatus.routeHistory} />
        </motion.div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-6">
            ...
            <div className="flex-1 text-center sm:text-left">
              ...
              <div className="flex items-center justify-center sm:justify-start gap-2 mb-2">
                ...
                <span className="text-gray-600 dark:text-gray-300">{deliveryStatus.courier.rating.toFixed(1)} ({deliveryStatus.courier.deliveriesCount} livraisons)</span>
              </div>
